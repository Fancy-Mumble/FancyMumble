...
`ChannelInfoPanel` counts the same way, so the header's number and the panel's
never disagree about the same room; what the panel adds is the *names* behind
it, which is the thing neither the header nor the roster can say.


## The channel's own panel

`ChannelInfoPanel` is the third occupant of the right-hand slot, beside the
roster and the server details. All three are one `surface` value, so opening
one closes the others rather than stacking 320px asides on a narrowing chat.
