...
describe("NebulaApp", () => {
  ...
  describe("the channel's own panel", () => {
    it("opens beside the conversation from the header menu", async () => {
      await connectedClient();
      fireEvent.click(screen.getByLabelText("Channel menu"));
      fireEvent.click(await screen.findByRole("menuitem", { name: "Channel info" }));

      const panel = await screen.findByRole("complementary", { name: "Channel info" });
      expect(within(panel).getByText("No description")).toBeTruthy();
    });

    it("gives way to the server's panel rather than stacking two asides", async () => {
      await connectedClient();
      fireEvent.click(screen.getByLabelText("Channel menu"));
      fireEvent.click(await screen.findByRole("menuitem", { name: "Channel info" }));
      await screen.findByRole("complementary", { name: "Channel info" });

      fireEvent.click(screen.getByLabelText("Channel menu"));
      fireEvent.click(await screen.findByRole("menuitem", { name: "Server Info" }));
      await screen.findByRole("complementary", { name: "Server info" });
      expect(screen.queryByRole("complementary", { name: "Channel info" })).toBeNull();
    });
  });
