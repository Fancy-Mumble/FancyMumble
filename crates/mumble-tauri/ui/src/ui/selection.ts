...
import { isUiDesignId, resolveUiDesign, UI_DESIGN_QUERY_PARAMETER } from "./registry";

export function getUiDesignOverride(search = globalThis.location?.search ?? ""): UiDesignId | null {
  const value = new URLSearchParams(search).get(UI_DESIGN_QUERY_PARAMETER);
  return isUiDesignId(value) ? value : null;
}

export async function getSelectedUiDesign(search = globalThis.location?.search ?? ""): Promise<UiDesignId> {
