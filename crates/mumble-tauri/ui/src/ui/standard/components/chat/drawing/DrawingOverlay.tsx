...
globalThis.addEventListener("drawing:clear-sender", (event: Event) => {
  const detail = (event as CustomEvent<{ senderSession?: unknown }>).detail;
  if (typeof detail?.senderSession === "number") {
    clearStrokesFromSender(detail.senderSession);
  }
});
