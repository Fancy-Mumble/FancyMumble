...
describe("ModernChannelList", () => {
  it("renders channel names", () => {
    ...
    render(<ModernChannelList {...BASE_PROPS} channels={channels} users={[]} />);
  ...
  it("sorts populated channels before empty ones", () => {
    ...
    render(<ModernChannelList {...BASE_PROPS} channels={channels} users={users} />);

    const buttons = screen.getAllByRole("button").filter((b) => b.textContent?.includes("Channel"));
  ...
  it("calls onSelectChannel on click", () => {
    ...
    render(<ModernChannelList {...BASE_PROPS} channels={channels} users={[]} onSelectChannel={onSelect} />);
  ...
  it("calls onJoinChannel on double-click", () => {
    ...
    render(<ModernChannelList {...BASE_PROPS} channels={channels} users={[]} onJoinChannel={onJoin} />);
  ...
  it("shows unread badge", () => {
    ...
    render(<ModernChannelList {...BASE_PROPS} channels={channels} users={[]} unreadCounts={{ 1: 5 }} />);
  ...
  it("shows mute/deaf icons for members", () => {
    ...
    const users = [makeUser({ session: 1, name: "Muted", channel_id: 1, self_mute: true, self_deaf: true })];
    const { container } = render(<ModernChannelList {...BASE_PROPS} channels={channels} users={users} />);
  ...
  it("wraps non-current channels in SwipeableCard on mobile", async () => {
    ...
    const { isMobilePlatform } = await import("@core/utils/platform");
    ...
    render(<ModernChannelList {...BASE_PROPS} channels={channels} users={[]} currentChannel={1} />);
