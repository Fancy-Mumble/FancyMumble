...
import { ImageIcon, PlayIcon, VolumeIcon, FileTextIcon, FolderIcon, FileIcon, CloseIcon } from "../../icons";
import { categorize, isPreviewable, type FileCategory } from "@core/features/fileserver/fileTypes";
...
export function CategoryIcon({ cat, size = 18 }: { cat: FileCategory; size?: number }) {
  switch (cat) {
    ...
    case "video":
      return <PlayIcon width={size} height={size} />;
    case "audio":
      return <VolumeIcon width={size} height={size} />;
    case "document":
      return <FileTextIcon width={size} height={size} />;
    case "archive":
...
export function FileThumb({
  ...
  useEffect(() => {
    ...
    const io = new IntersectionObserver((entries) => {
      ...
      io.disconnect();
      source
        .loadPreviewUrl(file.id, file.mime_type, THUMB_MAX_BYTES)
        .then((u) => {
  ...
  return (
    ...
    >
      {isImage && url && !failed ? <img src={url} alt="" loading="lazy" /> : <CategoryIcon cat={cat} />}
    </button>
  );
}
  ...
  if (state.kind === "loading")
    body = (
      <div className={styles.previewMsg}>
        {t("fileServer.previewLoading", { defaultValue: "Loading preview…" })}
      </div>
    );
  else if (state.kind === "error")
    body = (
      <div className={styles.previewMsg}>
        {t("fileServer.previewError", { defaultValue: "Preview failed" })}: {state.msg}
      </div>
    );
  else if (state.kind === "text") body = <pre className={styles.previewText}>{state.text}</pre>;
  else if (cat === "image")
    body = <img className={styles.previewMedia} src={state.url} alt={file.filename} />;
  else if (cat === "video")
    body = <video className={styles.previewMedia} src={state.url} controls autoPlay />;
  else if (cat === "audio") body = <audio src={state.url} controls autoPlay />;
  else if (mime === "application/pdf")
    body = <iframe className={styles.previewFrame} src={state.url} title={file.filename} />;
  else
    body = (
      <div className={styles.previewMsg}>
        {t("fileServer.previewUnavailable", { defaultValue: "No inline preview for this type." })}
      </div>
    );
    ...
    <div className={styles.previewBackdrop} onClick={onClose} role="presentation">
      ...
      >
        <div className={styles.previewHead}>
          <span className={styles.previewName} title={file.filename}>
            {file.filename}
          </span>
          ...
          <button
            ...
            onClick={onClose}
            aria-label={t("fileServer.previewClose", { defaultValue: "Close" })}
          >
