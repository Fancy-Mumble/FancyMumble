...
import { updatePreferences } from "@core/preferencesStorage";
import { DECORATIONS, NAMEPLATES, EFFECTS, AVATAR_BORDERS } from "@core/features/settings/profileData";
  ...
  return (
    <>
            ...
            >
              {identities.map((label) => (
                <option key={label} value={label}>
                  {label}
                  {label === connectedCertLabel ? t("profile.connectedSuffix") : ""}
                </option>
              ))}
            </select>
            <button type="button" className={styles.ghostBtn} onClick={onGoToIdentities}>
              {t("profile.manageIdentities")}
            </button>
          </div>
          {connectedCertLabel && activeIdentity !== connectedCertLabel && (
            <p className={panelStyles.infoBoxYellow}>{t("profile.viewingOtherIdentity")}</p>
          )}
        </section>
      )}
      ...
      <section className={styles.section}>
        ...
        <p className={styles.fieldHint}>{t("profile.usernameHint")}</p>
        <input
      ...
      <section className={styles.section}>
        ...
        <p className={styles.fieldHint}>{t("profile.avatarHint")}</p>
        <div className={panelStyles.avatarRow}>
          {avatar && (
            <img src={avatar} alt={t("profile.avatarThumbAlt")} className={panelStyles.avatarThumb} />
          )}
          <button type="button" className={styles.ghostBtn} onClick={() => setShowAvatarEditor(true)}>
      ...
      <section className={styles.section}>
        ...
        <p className={styles.fieldHint}>{t("profile.bannerHint")}</p>
        ...
        {!profile.banner?.image && profile.banner?.color && (
          <div className={panelStyles.bannerThumb} style={{ background: profile.banner.color, height: 60 }} />
        )}
        <button type="button" className={styles.ghostBtn} onClick={() => setShowBannerEditor(true)}>
      ...
      <section className={styles.section}>
        ...
        <p className={styles.fieldHint}>{t("profile.bioHint")}</p>
      ...
      <section className={styles.section}>
        ...
        <p className={styles.fieldHint}>{t("profile.statusHint")}</p>
        <input
          className={styles.input}
          type="text"
          autoComplete="off"
      ...
      <section className={styles.section}>
        ...
        <p className={styles.fieldHint}>{t("profile.cardBgHint")}</p>
        <CardColorPicker
      ...
      <section className={styles.section}>
        ...
        <p className={styles.fieldHint}>{t("profile.avatarBorderHint")}</p>
        <div className={styles.optionGrid}>
          {AVATAR_BORDERS.filter((ab) => ab.id !== "custom" || isExpert).map((ab) => {
            ...
            return (
              <button
                ...
                className={`${panelStyles.avatarBorderCard} ${
                  (profile.avatarBorder ?? "default") === ab.id ? styles.optionCardSelected : ""
                }`}
                onClick={() =>
                  onPatchProfile({
        ...
        {isExpert && profile.avatarBorder === "custom" && (
          <div className={styles.field} style={{ marginTop: 8 }}>
            ...
            <input
              ...
              onChange={(e) => onPatchProfile({ avatarBorderCustom: e.target.value || undefined })}
      ...
      <section className={styles.section}>
        ...
        <p className={styles.fieldHint}>{t("profile.decorationHint")}</p>
        <div className={styles.optionGrid}>
          {DECORATIONS.map((d) => (
            <button
              key={d.id}
              ...
              className={`${styles.optionCard} ${
                (profile.decoration ?? "none") === d.id ? styles.optionCardSelected : ""
              }`}
              onClick={() =>
                onPatchProfile({
      ...
      <section className={styles.section}>
        ...
        <p className={styles.fieldHint}>{t("profile.nameplateHint")}</p>
        <div className={styles.optionGrid}>
          {NAMEPLATES.map((n) => (
            <button
              key={n.id}
              ...
              className={`${panelStyles.nameplateCard} ${
                (profile.nameplate ?? "none") === n.id ? styles.optionCardSelected : ""
              }`}
      ...
      <section className={styles.section}>
        ...
        <p className={styles.fieldHint}>{t("profile.effectHint")}</p>
        <div className={styles.optionGrid}>
          {EFFECTS.map((fx) => (
            <button
              key={fx.id}
              type="button"
              className={`${styles.optionCard} ${
                (profile.effect ?? "none") === fx.id ? styles.optionCardSelected : ""
              }`}
              onClick={() =>
                onPatchProfile({
      ...
      {/* -- Name Style ----------------------------------------- */}
      <NameStyleSection nameStyle={nameStyle} onPatch={patchNameStyle} displayName={defaultUsername} />
