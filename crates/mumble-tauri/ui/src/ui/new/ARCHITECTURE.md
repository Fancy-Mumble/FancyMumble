...
1. `NewClientApp` owns application state and selects screens and surfaces.
2. Screen components (`OnboardingFlow`, `ServerRail`, `AppTitleBar`) compose feature components.
3. Feature components (`ChannelRow`, `MemberRow`, `MessageItem`, `LinkPreviews`, and the modal surfaces) compose shared primitives.
4. Primitives (`Button`, `IconButton`, `TextField`, `SearchField`, `ModalSurface`, and `Stepper`) are the only layer that creates native interactive HTML controls.
