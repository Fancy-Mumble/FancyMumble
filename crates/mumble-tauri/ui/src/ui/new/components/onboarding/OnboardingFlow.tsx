...
export default function OnboardingFlow({ onComplete }: OnboardingFlowProps) {
  ...
  return <section className={styles.onboarding}>
    ...
    <div className={styles.stepContent}>
      ...
      {step === 1 && <div className={styles.formStep}><small>STEP 2 OF 4</small><h2>How should people see you?</h2><p>You can change this independently for every saved server later.</p><TextField label="Display name" autoFocus value={username} onChange={(event) => setUsername(event.target.value)} placeholder="Your display name" /></div>}
      {step === 2 && <div className={styles.formStep}><small>STEP 3 OF 4</small><h2>Add your first server</h2><p>Enter the connection details provided by your community or team.</p><TextField label="Server name" hint="Optional" value={label} onChange={(event) => setLabel(event.target.value)} placeholder="My community" /><TextField label="Server address" autoFocus value={host} onChange={(event) => setHost(event.target.value)} placeholder="voice.example.com" /><TextField label="Port" type="number" min={1} max={65535} value={port} onChange={(event) => setPort(Number(event.target.value))} /></div>}
      {step === 3 && <div className={styles.readyStep}><span><CheckIcon /></span><h2>You’re ready, {username}.</h2><p><strong>{label.trim() || host}</strong> will be saved to your server sidebar. Connect now or explore the launcher first.</p><div><Button disabled={saving} onClick={() => void finish(false)}>Finish setup</Button><Button variant="primary" disabled={saving} onClick={() => void finish(true)}>{saving ? "Saving…" : "Save & connect"}</Button></div></div>}
    </div>
    {step < 3 && <footer><Button disabled={step === 0} onClick={() => setStep((current) => current - 1)}>Back</Button><span>{step + 1} / {STEPS.length}</span><Button variant="primary" disabled={!canContinue} onClick={() => setStep((current) => current + 1)}>Continue</Button></footer>}
  </section>;
}
