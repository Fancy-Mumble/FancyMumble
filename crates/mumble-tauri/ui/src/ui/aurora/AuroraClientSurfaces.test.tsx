...
describe("ScreenSharePanel", () => {
  ...
  it("subscribes to the broadcasting set without producing unstable snapshots", async () => {
    ...
    await waitFor(() => expect(invokeMock).toHaveBeenCalledWith("capture_source_thumbnail", expect.objectContaining({ kind: "screen", id: 1 })));
  });
  ...
  it("combines a display and camera and forwards the selected encoder settings", async () => {
    ...
    render(<ScreenSharePanel onClose={vi.fn()} controller={{ settings: { maxDimension: 1920, maxFps: 60 }, confirmSource, stopSharing: vi.fn() }} />);
    ...
    await waitFor(() => expect(confirmSource).toHaveBeenCalledWith([{ kind: "screen", id: 1 }, { kind: "device", id: 8 }], { maxDimension: 1920, maxFps: 60 }));
  });
});
