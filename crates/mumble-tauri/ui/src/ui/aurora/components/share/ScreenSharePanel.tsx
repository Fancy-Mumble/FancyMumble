...
import { QUALITY_PRESETS, RESOLUTIONS, FRAME_RATES, presetOf, resolutionKeyOf, type QualityPreset, type StreamSettings } from "@core/features/chat/stream/streamSettings";
...
export interface ScreenShareController {
  ...
  confirmSource: (sources: readonly SelectedSource[], settings: StreamSettings, options?: { reuseDisplay?: boolean }) => Promise<void>;
  stopSharing: () => void;
}
...
const sharePayload = (sources: readonly CaptureSource[]) => JSON.stringify({ v: 1, tracks: sources.map((source, index) => ({ mid: String(index), content: source.kind === "device" ? "camera" : "screen" })) });
...
function initialStreamSettings(fallback: StreamSettings): StreamSettings {
  try {
    const parsed = JSON.parse(globalThis.localStorage.getItem(STREAM_SETTINGS_KEY) ?? "null") as Partial<StreamSettings> | null;
    return typeof parsed?.maxDimension === "number" && typeof parsed?.maxFps === "number" ? { maxDimension: parsed.maxDimension, maxFps: parsed.maxFps } : fallback;
...
export default function ScreenSharePanel({ onClose, controller }: { onClose: () => void; controller?: ScreenShareController }) {
  ...
  const [settings, setSettings] = useState<StreamSettings>(() => initialStreamSettings(controller?.settings ?? QUALITY_PRESETS.hd));
  ...
  useEffect(() => { try { globalThis.localStorage.setItem(STREAM_SETTINGS_KEY, JSON.stringify(settings)); } catch { /* Storage can be disabled by platform policy. */ } }, [settings]);
  ...
  const selectedSources = [selected, selectedCamera].filter((source): source is CaptureSource => source !== null && (source.kind !== "device" || selected?.kind !== "device" || source === selected));
  ...
  const start = async () => {
    ...
    try {
      if (controller) await controller.confirmSource(selectedSources.map(({ kind, id }) => ({ kind, id })), settings);
      else {
        await state.sendWebRtcSignal(0, 0, sharePayload(selectedSources), state.activeServerId);
        await invoke("start_screen_broadcast", { sources: selectedSources.map(({ kind, id }) => ({ kind, id })), serverId: state.activeServerId, maxDimension: settings.maxDimension, maxFps: settings.maxFps, reusePortalSource: false });
  ...
  return <ModalSurface title="Share your screen" eyebrow="SCREEN & CAMERA" onClose={onClose} className={styles.shareSurface}>
    <div className={styles.sharePicker}>
      ...
      <div className={panelStyles.shareQuality}><span><strong>Stream quality</strong><small>Resolution and frame rate sent to viewers</small></span>{(["sd", "hd", "source"] as QualityPreset[]).map((preset) => <Button variant={presetOf(settings) === preset ? "secondary" : "bare"} key={preset} onClick={() => setSettings(QUALITY_PRESETS[preset])}>{preset === "source" ? "Source" : preset.toUpperCase()}</Button>)}<select aria-label="Resolution" value={resolutionKeyOf(settings)} onChange={(event) => setSettings((current) => ({ ...current, maxDimension: RESOLUTIONS.find((item) => item.key === event.target.value)?.maxDimension ?? current.maxDimension }))}>{RESOLUTIONS.map((resolution) => <option value={resolution.key} key={resolution.key}>{resolution.key === "source" ? "Source" : resolution.key}</option>)}</select><select aria-label="Frame rate" value={settings.maxFps} onChange={(event) => setSettings((current) => ({ ...current, maxFps: Number(event.target.value) }))}>{FRAME_RATES.map((fps) => <option key={fps} value={fps}>{fps} FPS</option>)}</select></div>
      <div className={styles.shareSourceGrid}>{sources === null ? Array.from({ length: 6 }, (_, index) => <div className={styles.shareSourceSkeleton} key={index}><span /><i /></div>) : visibleSources.length > 0 ? visibleSources.map((source) => <SourceCard key={sourceKey(source)} source={source} thumbnail={thumbnails.get(sourceKey(source))} selected={source.kind === "device" ? selectedCamera?.id === source.id : selected?.kind === source.kind && selected.id === source.id} onSelect={() => pickSource(source)} />) : <div className={styles.shareEmpty}><span><EmptyIcon /></span><strong>No {tabs.find((item) => item.kind === tab)?.label.toLowerCase()} found</strong><small>Try another source category.</small></div>}</div>
      {error && <div className={styles.error}>{error}</div>}
      <footer className={styles.shareFooter}><div className={styles.shareSelection}>{primarySelection && SelectedIcon ? <><span><SelectedIcon /></span><div><small>READY TO SHARE</small><strong title={selectedSources.map((source) => source.title).join(" + ")}>{selectedSources.map((source) => source.title).join(" + ")}</strong></div><em>{settings.maxDimension === 0 ? "Source" : `${Math.round(settings.maxDimension / 16 * 9)}p`} · {settings.maxFps} FPS</em></> : <span>Select a source to continue</span>}</div><Button onClick={onClose}>Cancel</Button>{isSharing && <Button variant="danger" leadingIcon={<TrashIcon />} onClick={() => void stop()}>Stop sharing</Button>}<Button variant="primary" leadingIcon={<SparklesIcon />} disabled={selectedSources.length === 0 || busy} onClick={() => void start()}>{busy ? "Starting…" : isSharing ? "Update share" : "Go live"}</Button></footer>
    </div>
  </ModalSurface>;
}
