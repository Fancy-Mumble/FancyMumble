...
export default function ChannelPresence({ users, talkingSessions }: ChannelPresenceProps) {
  ...
  return <span className={styles.presence}>
    {users.slice(0, MAX_STACKED).map((user) => <ChannelPresenceAvatar key={user.session} name={user.name} talking={talkingSessions.has(user.session)} />)}
