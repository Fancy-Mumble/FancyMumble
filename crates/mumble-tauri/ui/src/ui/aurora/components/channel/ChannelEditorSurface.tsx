...
export default function ChannelEditorSurface({ channel, parentId, onClose }: ChannelEditorSurfaceProps) {
  ...
  useEffect(() => { if (sourceDescription != null) setDescription(sourceDescription); }, [sourceDescription]);
  ...
  const remove = async () => {
    ...
    setBusy(true); setError(null);
    try { await useAppStore.getState().deleteChannel(channel.id); onClose(); }
    catch (reason) { setError(String(reason)); setBusy(false); }
  };

  return <ModalSurface title={creating ? "Create channel" : `Edit #${channel.name}`} eyebrow="CHANNEL MANAGEMENT" onClose={onClose} className={styles.channelEditorSurface}>
    <form className={styles.channelEditor} onSubmit={(event) => { event.preventDefault(); void submit(); }}>
      <div className={styles.channelEditorGrid}>
        <TextField label="Channel name" value={name} onChange={(event) => setName(event.target.value)} autoFocus required />
        <TextField label="Position" type="number" min={0} value={position} onChange={(event) => setPosition(Number(event.target.value))} />
        <label className={styles.selectField}>Parent channel<select value={selectedParentId} onChange={(event) => setSelectedParentId(Number(event.target.value))}>{channels.filter((candidate) => candidate.id !== channel?.id && !candidate.detached).map((candidate) => <option key={candidate.id} value={candidate.id}>{candidate.id === 0 ? "Root" : candidate.name}</option>)}</select></label>
        <TextField label="Maximum users" hint="0 means unlimited" type="number" min={0} value={maxUsers} onChange={(event) => setMaxUsers(Number(event.target.value))} />
        <TextField label="Password" hint={creating ? "Optional" : "Leave empty to keep unchanged"} type="password" value={password} onChange={(event) => setPassword(event.target.value)} />
      </div>
      <RichTextEditor label="Description" value={description} onChange={setDescription} placeholder="Describe what this channel is for" ariaLabel="Channel description" />
      <div className={styles.channelEditorGrid}>
        <label className={styles.selectField}>Message history<select value={pchatProtocol} onChange={(event) => setPchatProtocol(event.target.value as PchatProtocol)}><option value="none">Disabled</option><option value="fancy_v1_full_archive">Full archive</option><option value="signal_v1">Encrypted Signal history</option></select></label>
      ...
      <footer className={styles.channelEditorActions}>
        ...
        <Button onClick={onClose}>Cancel</Button><Button variant="primary" type="submit" disabled={busy || !name.trim()}>{busy ? "Saving…" : creating ? "Create channel" : "Save changes"}</Button>
      </footer>
    </form>
  </ModalSurface>;
}
