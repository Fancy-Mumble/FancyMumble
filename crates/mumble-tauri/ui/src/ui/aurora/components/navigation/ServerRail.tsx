...
export default function ServerRail({
  ...
  const renderGroup = (group: RailGroup): ReactNode => {
    ...
    const activeHere = group.identities.some(
      (identity) => identity.sessionId && identity.sessionId === activeSessionId,
    );
    const single = group.identities[0];

    return (
      <div className={styles.serverGroup} key={group.key}>
        <div className={styles.groupTile}>
          {multi && (
            <i className={styles.groupCount} aria-hidden="true">
          ...
          <RailEntry
            ...
            title={
              multi
                ? `${open ? "Collapse" : "Expand"} ${group.label} (${group.identities.length} identities)`
                : online
                  ? `Open ${group.label}`
                  : `Connect to ${group.label}`
            }
            primary={group.label}
        ...
        {open && (
          <div className={styles.groupIdentities}>
  ...
  return (
    <aside className={`${styles.servers} ${className ?? ""}`} aria-label={label} data-expanded={expanded}>
      <div className={styles.railHeader}>
        <span>{activeSessionId ? "CONNECTED" : "YOUR SERVERS"}</span>
        <IconButton
          icon={expanded ? <ChevronLeftIcon /> : <ChevronRightIcon />}
          label={expanded ? "Collapse server sidebar" : "Expand server sidebar"}
          aria-expanded={expanded}
          onClick={onToggle}
        />
      </div>
      <div className={styles.serverLibrary}>{groups.map(renderGroup)}</div>
      <Button variant="ghost" className={styles.addServer} onClick={onAdd} leadingIcon={<PlusIcon />}>
        Add server
      </Button>
    </aside>
  );
