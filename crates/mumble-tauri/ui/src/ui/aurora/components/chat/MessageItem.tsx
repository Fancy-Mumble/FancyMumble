...
export default function MessageItem({
  ...
  const readers = message.message_id
    ? getReadersForMessage(message.channel_id, message.message_id, messageIds)
    : [];
  ...
  const saveEdit = async () => {
    if (!message.message_id || !editBody.trim()) return;
    await useAppStore.getState().editMessage(message.channel_id, message.message_id, editBody.trim());
    setEditing(false);
  };
  const remove = async () => {
  ...
  const quote = () =>
    globalThis.dispatchEvent(
      new CustomEvent("new-ui:quote-message", { detail: { sender: message.sender_name, body: safeBody } }),
    );
  const copy = () =>
    navigator.clipboard.writeText(
      new DOMParser().parseFromString(message.body, "text/html").body.textContent ?? message.body,
    );
  const select = () => {
    if (message.message_id) onToggleSelection?.(message.message_id);
    setContextMenu(null);
  };
  ...
  return (
    <article
      className={`${styles.message} ${message.is_own ? styles.ownMessage : ""} ${selected ? extensionStyles.messageSelected : ""}`}
    ...
    >
      {selectionMode && message.message_id && (
        <button
          type="button"
          className={extensionStyles.messageSelect}
      ...
      <div className={extensionStyles.messageContent}>
        <header>
          <strong>{message.sender_name || "Server"}</strong>
          <time>{formatTime(message.timestamp)}</time>
          {message.edited_at && <small>edited</small>}
          {message.pinned && <small>pinned</small>}
        </header>
        {editing ? (
          <div className={extensionStyles.messageEdit}>
            <textarea value={editBody} onChange={(event) => setEditBody(event.target.value)} autoFocus />
          ...
          <FileAttachmentMarker payload={filePayload} />
        ) : watchSessionId ? (
          <WatchTogetherCard
          ...
          <div className={extensionStyles.reactionRow}>
            {reactions.map((reaction) => (
              <Button
                variant="bare"
                key={reaction.emoji}
                title={[...reaction.reactorHashNames.values()].join(", ")}
                onClick={() => void toggleReaction(reaction.emoji)}
              >
                {reaction.emoji} {reaction.reactorHashes.size}
              </Button>
            ))}
          </div>
        )}
        {message.is_own && readers.length > 0 && (
          <small className={extensionStyles.readers} title={readers.map((reader) => reader.name).join(", ")}>
            Read by {readers.length}
          </small>
        )}
        <div className={extensionStyles.messageActions}>
          <Button variant="bare" onClick={() => void toggleReaction("👍")}>
            👍
          </Button>
          <Button variant="bare" onClick={() => void toggleReaction("❤️")}>
            ❤️
          </Button>
          <IconButton icon={<QuoteIcon />} label="Quote message" onClick={quote} />
          <IconButton icon={<CopyIcon />} label="Copy message" onClick={() => void copy()} />
          {message.message_id && (
            <IconButton
              icon={<PinIcon />}
          ...
          {message.is_own && message.message_id && (
            <>
              <IconButton icon={<EditIcon />} label="Edit message" onClick={() => setEditing(true)} />
              <IconButton
                icon={<TrashIcon />}
        ...
        {confirmDelete && (
          <div className={extensionStyles.deleteMessageConfirm}>
      ...
      {contextMenu && (
        <div
          className={extensionStyles.messageContextMenu}
        ...
        >
          ...
          <Button variant="bare" leadingIcon={<CheckIcon />} onClick={select}>
