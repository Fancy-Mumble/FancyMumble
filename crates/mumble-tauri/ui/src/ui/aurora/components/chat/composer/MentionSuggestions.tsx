...
function candidateLabel(candidate: MentionCandidate): string {
  switch (candidate.kind) {
    case "user": return candidate.name;
    case "role": return `@${candidate.name}`;
...
function candidateHint(candidate: MentionCandidate): string {
  switch (candidate.kind) {
    ...
    case "everyone": return "Everyone in the channel";
    case "here": return "Everyone online";
  }
}

function MentionRow({ candidate, active, onPick }: {
  ...
  return <Button
    ...
    onClick={onPick}
  >
    <strong>{candidateLabel(candidate)}</strong><small>{candidateHint(candidate)}</small>
  </Button>;
}
...
export default function MentionSuggestions({ candidates, activeIndex, onPick }: MentionSuggestionsProps) {
  ...
  return <div className={styles.suggestions} role="listbox" aria-label="Mention suggestions">
    {candidates.map((candidate, index) => <MentionRow
      key={candidate.kind === "user" ? `user-${candidate.session}` : candidate.kind === "role" ? `role-${candidate.name}` : candidate.kind}
      candidate={candidate}
