...
import { Button, GifPicker, IconButton, MentionSuggestions, SlashSuggestions, useComposerAutocomplete } from "../../components";
...
import { AttachIcon, BoldIcon, CodeIcon, EmojiPlusIcon, ImageIcon, Link2Icon, RadioIcon, SendIcon, UsersGroupIcon } from "@ui/icons";
...
export function RichComposer({ channel, targetLabel, onSend }: { channel: ChannelEntry | null; targetLabel?: string; onSend: (html: string) => Promise<void> }) {
  ...
  const send = async () => {
    ...
    if (autocomplete.submitSlashCommand()) { resetTyping(); return; }
    setSending(true);
  ...
  const pasteImages = (clipboard: DataTransfer | null): boolean => {
    ...
    void (async () => { for (const image of images) await attachImage(image); })();
    return true;
  };
  ...
  return <div className={extensionStyles.composer}>
    ...
    <SlashSuggestions entries={autocomplete.slashEntries} activeIndex={autocomplete.slashIndex} onPick={autocomplete.pickSlash} />
    <MentionSuggestions candidates={autocomplete.mentionCandidates} activeIndex={autocomplete.mentionIndex} onPick={autocomplete.pickMention} />
    ...
    <div className={extensionStyles.composerBar}>
      <input ref={fileInput} type="file" accept="image/*" multiple hidden onChange={(event) => { void attachImages(event.target.files); event.target.value = ""; }} />
      ...
      <EditorContent
        ...
        onPaste={(event) => { if (pasteImages(event.clipboardData)) event.preventDefault(); }}
        onKeyDown={(event) => {
