...
function Toggle({ checked, onChange, label }: { checked: boolean; onChange: () => void; label: string }) {
  return (
    <Button
      className={checked ? styles.audioToggleOn : styles.audioToggle}
      role="switch"
    ...
    <div className={styles.audioSettings}>
      {captureError && (
        <div className={styles.formError} role="alert">
          Microphone unavailable: {captureError}
        </div>
      )}
      <SettingRow title="Microphone" detail="Input device used for voice transmission.">
        <select
      ...
      <SettingRow title="Speakers" detail="Output device used for incoming voice.">
        <select
      ...
      <SettingRow title="Microphone volume" detail="Gain applied before voice processing.">
        <Slider
      ...
      <SettingRow title="Speaker volume" detail="Master level for incoming voice.">
        <Slider
      ...
      <SettingRow title="Activation mode" detail="Choose automatic voice activity or push-to-talk.">
        <select
          value={settings.push_to_talk ? "ptt" : "vad"}
          onChange={(event) => void patch({ push_to_talk: event.target.value === "ptt" })}
        >
      ...
      {settings.push_to_talk ? (
        <SettingRow title="Push-to-talk shortcut" detail="Global shortcut recognized by the native client.">
          <input
        ...
        <>
          <SettingRow title="Voice activation threshold" detail="The microphone opens above this level.">
            <Slider
          ...
          <SettingRow title="Automatic input sensitivity" detail="Adapt the threshold to ambient noise.">
            <Toggle
              label="Automatic input sensitivity"
              checked={settings.auto_input_sensitivity}
              onChange={() => void patch({ auto_input_sensitivity: !settings.auto_input_sensitivity })}
            />
          </SettingRow>
        </>
      )}
      <SettingRow title="Noise suppression" detail="Remove steady background noise before transmission.">
        <select
          value={settings.denoiser_algorithm}
          onChange={(event) =>
            void patch({
              denoiser_algorithm: event.target.value as NoiseSuppressionAlgorithm,
              noise_suppression: event.target.value !== "none",
            })
          }
        >
          {algorithms.map((algorithm) => (
            <option key={algorithm} value={algorithm}>
              {NOISE_SUPPRESSION_LABELS[algorithm]}
            </option>
          ))}
        </select>
      </SettingRow>
      {paramSpecs.map((spec) => (
        <SettingRow key={spec.id} title={spec.label} detail={spec.description}>
          <Slider
            ...
            format={(value) => `${value}${spec.unit ? ` ${spec.unit}` : ""}`}
          />
        </SettingRow>
      ))}
      <SettingRow title="Automatic gain" detail="Normalize quiet and loud microphones.">
        <Toggle
          label="Automatic gain"
          checked={settings.auto_gain}
          onChange={() => void patch({ auto_gain: !settings.auto_gain })}
        />
      </SettingRow>
      <SettingRow title="Maximum gain" detail="Upper automatic-gain boost.">
        <Slider
      ...
      <SettingRow title="Gate close ratio" detail="How far the signal must fall before transmission closes.">
        <Slider
      ...
      <SettingRow title="Gate hold" detail="How long the voice gate remains open after speech.">
        <Slider
      ...
      <SettingRow title="Voice bitrate" detail="Higher values improve quality and use more bandwidth.">
        <Slider
      ...
      <SettingRow title="Packet duration" detail="Short frames reduce latency; long frames reduce overhead.">
        <select
          value={settings.frame_size_ms}
          onChange={(event) => void patch({ frame_size_ms: Number(event.target.value) })}
        >
      ...
      <SettingRow title="Force TCP audio" detail="Tunnel voice through TCP for restrictive networks.">
        <Toggle
          label="Force TCP audio"
          checked={settings.force_tcp_audio}
          onChange={() => void patch({ force_tcp_audio: !settings.force_tcp_audio })}
        />
      </SettingRow>
      <SettingRow title="Exclusive microphone" detail="Request exclusive device access on supported systems.">
        <Toggle
          label="Exclusive microphone"
          checked={settings.exclusive_input ?? false}
          onChange={() => void patch({ exclusive_input: !settings.exclusive_input })}
        />
      </SettingRow>
      <SettingRow
        title="Playback backend"
        detail="Switch the native output implementation; applies on the next voice start."
      >
        <select
          value={rodioBackend ? "rodio" : "cpal"}
          onChange={(event) => {
            const useRodio = event.target.value === "rodio";
            setRodioBackend(useRodio);
            void invoke("set_audio_backend", { useRodio });
          }}
        >
          <option value="rodio">Rodio (recommended)</option>
          <option value="cpal">CPAL compatibility</option>
        </select>
      </SettingRow>
      <div className={styles.micTest}>
        <div>
          <strong>Microphone test</strong>
          <small>Measure the processed input without transmitting.</small>
          <i>
            <b style={{ width: `${micLevel * 100}%` }} />
          </i>
        </div>
      ...
      <div className={styles.micTest}>
        <div>
          <strong>Hear your processed voice</strong>
          <small>Record a short sample, then play it back through the selected output device.</small>
          <i>
            <b
              style={{
                width: `${replay.phase === "recording" ? Math.min(100, (replay.elapsed_ms / Math.max(1, replay.capacity_ms)) * 100) : replay.phase === "playing" ? Math.min(100, (replay.elapsed_ms / Math.max(1, replay.total_ms)) * 100) : 0}%`,
              }}
            />
          </i>
        </div>
