    ...
    <>
      ...
      <div className={styles.form}>
        <TextField
          ...
          value={prefs.defaultUsername}
          onChange={(event) => onLocalChange({ defaultUsername: event.target.value })}
          onBlur={() => void onPatch({ defaultUsername: prefs.defaultUsername })}
        />
        <SelectField
          label="Feature level"
          value={prefs.userMode}
          options={FEATURE_LEVELS}
          onChange={(event) => void onPatch({ userMode: event.target.value as UserPreferences["userMode"] })}
        />
      </div>
    </>
  );
