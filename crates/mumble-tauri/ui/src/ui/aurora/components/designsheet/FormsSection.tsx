...
export default function FormsSection() {
  ...
  return (
    <Section
      id="forms"
      eyebrow="03 / Forms"
      title="Input, without friction"
      description="Accessible fields for server setup, profiles, permissions, search, security, and deep preference screens."
    >
      <div className={layout.specimenGrid}>
        <Specimen title="Text fields" meta="Input states" wide>
          <div className={forms.formGrid}>
            <label className={fields.field}>
              ...
              <input defaultValue="Morgan Oakes" />
              <small>This is how others see you.</small>
            </label>
            <label className={fields.field}>
              <span>Server address</span>
              <div className={fields.inputWithIcon}>
                <Server size={16} />
                <input placeholder="voice.example.com" />
              </div>
            </label>
            <label className={`${fields.field} ${fields.fieldError}`}>
              <span>Password</span>
              <div className={fields.inputWithIcon}>
                <LockKeyhole size={16} />
                <input type="password" defaultValue="incorrect" />
              </div>
              <small>That password doesn’t match.</small>
            </label>
            <label className={fields.field}>
              <span>Search</span>
              <div className={fields.inputWithIcon}>
                <Search size={16} />
                <input placeholder="Messages, people, files…" />
                <kbd>⌘ K</kbd>
              </div>
            </label>
          </div>
        ...
        <Specimen title="Selection" meta="Checkbox / radio">
          <div className={forms.optionStack}>
            {["Balanced", "Voice clarity", "Studio quality"].map((option) => (
              <label key={option} className={forms.radioRow}>
                <input
                  type="radio"
        ...
        <Specimen title="Switches & slider" meta="Preferences">
          <div className={forms.settingRow}>
            <span>
              <strong>Noise suppression</strong>
              <small>Remove background sound</small>
            </span>
            <button
              type="button"
              role="switch"
              aria-checked={toggleOn}
              className={toggleOn ? forms.switchOn : forms.switch}
              onClick={() => setToggleOn((value) => !value)}
            >
              <i />
            </button>
          </div>
          <label className={forms.range}>
            <span>
              <Volume2 size={15} /> Output volume <b>72%</b>
            </span>
        ...
        <Specimen title="Select & tags" meta="Structured input" wide>
          <div className={forms.formGrid}>
            <label className={fields.field}>
              ...
              <div className={fields.selectLike}>
                Studio microphone <ChevronDown size={15} />
              </div>
            </label>
            <label className={fields.field}>
              <span>Roles</span>
              <div className={`${forms.tagInput} ${fields.fieldTagInput}`}>
                <span>
                  Designer <X size={12} />
                </span>
                <span>
