...
export default function ActionsSection() {
  ...
  return (
    <Section
      id="actions"
      eyebrow="02 / Actions"
      title="Buttons with intent"
      description="A small hierarchy of clear actions replaces one-off button treatments across dialogs, chat, calls, and settings."
    >
      <div className={layout.specimenGrid}>
        <Specimen title="Button hierarchy" meta="Default states" wide>
          <div className={layout.wrapRow}>
            ...
            <button type="button" className={controls.ghostButton}>
              <Settings2 size={15} /> Configure
            </button>
            <button type="button" className={controls.dangerButton}>
              <Trash2 size={15} /> Delete
            </button>
            <button type="button" className={controls.secondaryButton} disabled>
              Unavailable
            </button>
          </div>
        </Specimen>
        <Specimen title="Icon actions" meta="Compact controls">
          <div className={layout.wrapRow}>
            <button type="button" className={controls.iconButton}>
              <Search size={17} />
            </button>
            <button type="button" className={controls.iconButton}>
              <Paperclip size={17} />
            ...
            <button type="button" className={controls.floatingButton}>
              <Plus size={20} />
            </button>
          </div>
        </Specimen>
        <Specimen title="Split & segmented" meta="Related actions">
          <div className={controls.splitButton}>
            <button type="button">Join voice</button>
            <button type="button" aria-label="More join options">
              <ChevronDown size={15} />
            </button>
          </div>
          <div className={controls.segmented}>
            {["Overview", "Members", "Files"].map((item) => (
              <button
                type="button"
                key={item}
                className={tab === item ? controls.segmentActive : ""}
                onClick={() => setTab(item)}
              >
                {item}
              </button>
