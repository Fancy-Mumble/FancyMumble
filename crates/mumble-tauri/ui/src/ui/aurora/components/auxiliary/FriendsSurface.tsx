...
export default function FriendsSurface({ onClose }: { onClose: () => void }) {
  ...
  const visibleFriends = useMemo(() => {
    ...
    return friends
      .filter(
        (friend) =>
          !needle || `${friend.userName} ${friend.serverLabel ?? ""}`.toLocaleLowerCase().includes(needle),
      )
      .sort(
        (left, right) =>
    ...
    try {
      ...
      if (!serverId && friend.serverHost && friend.serverPort) {
        const saved = (await getSavedServers()).find(
          (server) =>
            server.host === friend.serverHost &&
            server.port === friend.serverPort &&
            server.username === friend.serverUsername,
        );
        await useAppStore
          .getState()
          .connect(
    ...
    >
      <div className={styles.toolbar}>
        <SearchField
          ...
          placeholder="Find a friend or server"
          aria-label="Search friends"
        />
        <span>
          <b>{friends.length}</b> saved
        </span>
      </div>
        ...
        {visibleFriends.map((friend) => {
          ...
          const connected =
            ...
            sessions.some(
              (session) =>
                session.status === "connected" &&
                friend.serverHost === session.host &&
            ...
            <article key={friend.id} className={styles.friend}>
              ...
              >
                <span className={styles.avatar}>
                  {friend.avatar ? (
                    <img src={`data:image/png;base64,${friend.avatar}`} alt="" />
                  ) : (
                    initials(friend.userName)
                  )}
                </span>
                <span className={styles.identity}>
                  <strong>{match?.userName ?? friend.userName}</strong>
                  <small>
                    {friend.serverLabel ??
                      (friend.serverHost
                        ? `${friend.serverHost}:${friend.serverPort ?? 64738}`
                        : "Unknown server")}
                  </small>
                </span>
                <span
          ...
          <div className={styles.empty}>
            ...
            <p>
              {query
                ...
                : "Open a user profile and choose Add friend. They will appear here even when offline."}
            </p>
