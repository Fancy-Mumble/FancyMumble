...
import { HashIcon, PinIcon, SearchIcon, SettingsIcon, TrashIcon, VolumeIcon, InfoIcon, WebcamIcon } from "@ui/icons";
...
import { ClientTitleBar, Button, ChannelContextMenu, ChannelEditorSurface, ChannelJoinPrompt, ChannelSidebar, MoveUsersDialog, PurgeChannelDialog, ConnectionOverlays, FriendsSurface, IconButton, MemberSidebar, MessageItem, AuroraClientRuntime, OnboardingFlow, PinnedMessagesPanel, PollCreatorSurface, QuickSwitcher, ScreenShareRuntime, SearchField, SidebarContextMenu, ServerAdminPanel, ServerRail, SessionStatusScreen, SettingsPanel, TypingStatus, WorkspaceSurface } from "./components";
...
import { filterChannelMessages, filterDmMessages, filterVisibleChannels, groupServersForRail, listChannelMembers } from "./clientSelectors";
import { useAuroraAppearance, useClientEventBridge, useClientPreferences, useMessageImageClicks, useRegisteredUsers, useSavedServers, useServerConfigSync, useShortcutBindings, useUserRelations } from "./clientHooks";
import { useChannelModeration, useChatSearch, useMemberDirectory, useRailExpansion, useSurfaceRouting } from "./clientState";
...
export default function AuroraClientApp() {
  ...
  const { surface, setSurface, marketplacePluginId, openMarketplace, quickSwitcherOpen, setQuickSwitcherOpen, showPollCreator, setShowPollCreator } = useSurfaceRouting();
  const { launcherRailExpanded, setLauncherRailExpanded, connectedRailExpanded, setConnectedRailExpanded } = useRailExpansion();
  const { channelEditor, setChannelEditor, channelMenu, setChannelMenu, sidebarMenu, setSidebarMenu, moveUsersSource, setMoveUsersSource, purgeChannel, setPurgeChannel, restrictedChannel, setRestrictedChannel } = useChannelModeration();
  const { chatSearchOpen, setChatSearchOpen, chatQuery, setChatQuery, showPinned, setShowPinned, selectedMessageIds, setSelectedMessageIds, toggleMessageSelection } = useChatSearch(selectedChannel, selectedDmUser);
  ...
  const attachMessageList = useMessageImageClicks(useCallback((src: string) => lightboxRef.current?.open(src), []));
  ...
  const serverGroups = useMemo(
    () => groupServersForRail(savedServers, sessions),
  ...
  const visibleChannels = useMemo(
    () => filterVisibleChannels({ channels, query: channelQuery, hideEmpty: hideEmptyChannels, currentChannel, selectedChannel }),
    [channelQuery, channels, hideEmptyChannels, currentChannel, selectedChannel],
  );
  const channelUsers = useMemo(
    () => listChannelMembers({ users, registeredUsers, scope: memberScope, query: memberQuery, selectedChannel, talkingSessions }),
  ...
  const orderedChannelIds = useMemo(() => [...channels].filter((channel) => !isStructuralChannel(channel)).sort((left, right) => left.position - right.position || left.name.localeCompare(right.name)).map((channel) => channel.id), [channels]);
  ...
  if (status === "disconnected" && sessions.length === 0) {
    if (savedServers === null || savedServers.length === 0) {
      return (
        <div className={`${styles.root} ${styles.onboardingRoot}`} data-testid="aurora-client-root">
          ...
          <main className={styles.onboardingPage}>{savedServers === null ? <span className={styles.spinner} /> : <OnboardingFlow onComplete={(server, connectNow) => { reloadSavedServers(); if (connectNow) void connectSaved(server); }} />}</main>
    ...
    return (
      <div className={`${styles.root} ${styles.launcherRoot} ${launcherRailExpanded ? styles.serverRailExpanded : ""}`} data-testid="aurora-client-root">
        <ClientTitleBar onOpenSettings={() => setSurface("settings")} />
        <ServerRail className={styles.railSlot} groups={serverGroups} expanded={launcherRailExpanded} connecting={connecting} label="Saved servers" onToggle={() => setLauncherRailExpanded((value) => !value)} onSelect={selectRailIdentity} onAdd={() => setSurface("servers")} />
  ...
  if (status === "disconnected") {
    return (
      <div className={`${styles.root} ${styles.launcherRoot} ${connectedRailExpanded ? styles.serverRailExpanded : ""}`} data-testid="aurora-client-root">
        <ClientTitleBar serverTitle={activeSession?.label ?? "Disconnected"} onOpenSettings={() => setSurface("settings")} />
        <ServerRail className={styles.railSlot} groups={serverGroups} expanded={connectedRailExpanded} activeSessionId={activeServerId} connecting={connecting} label="Servers" onToggle={() => setConnectedRailExpanded((value) => !value)} onSelect={selectRailIdentity} onAdd={() => setSurface("servers")} />
  ...
  return (
    <div className={`${styles.root} ${connectedRailExpanded ? styles.serverRailExpanded : ""}`} data-testid="aurora-client-root">
      ...
      <ServerRail className={styles.railSlot} groups={serverGroups} expanded={connectedRailExpanded} activeSessionId={activeServerId} label="Connected servers" onToggle={() => setConnectedRailExpanded((value) => !value)} onSelect={selectRailIdentity} onAdd={() => setSurface("servers")} />
      ...
      <ChannelSidebar
        ...
        onJoinChannel={(channel) => channel.is_enter_restricted ? setRestrictedChannel(channel) : void useAppStore.getState().joinChannel(channel.id)}
        onChannelContextMenu={(channel, event) => { setSidebarMenu(null); setChannelMenu({ channel, x: event.clientX, y: event.clientY }); }}
        onSidebarContextMenu={(position) => { setChannelMenu(null); setSidebarMenu(position); }}
      />
      ...
      <MemberSidebar members={channelUsers} scope={memberScope} onScopeChange={setMemberScope} query={memberQuery} onQueryChange={setMemberQuery} ownSession={ownSession} talkingSessions={talkingSessions} onHoverMember={setHoveredUser} />
      ...
      {channelEditor && <ChannelEditorSurface channel={channelEditor.channel} parentId={channelEditor.parentId} initialStructural={channelEditor.structural} onClose={() => setChannelEditor(null)} />}
      {restrictedChannel && <ChannelJoinPrompt channel={restrictedChannel} onClose={() => setRestrictedChannel(null)} />}
      {sidebarMenu && <SidebarContextMenu x={sidebarMenu.x} y={sidebarMenu.y} onCreateChannel={() => { setChannelEditor({ channel: null, parentId: 0 }); setSidebarMenu(null); }} onCreateCategory={() => { setChannelEditor({ channel: null, parentId: 0, structural: true }); setSidebarMenu(null); }} />}
      {channelMenu && <ChannelContextMenu
        ...
        onOpenText={() => { void useAppStore.getState().selectChannel(channelMenu.channel.id); setChannelMenu(null); }}
        onJoinVoice={() => { if (channelMenu.channel.is_enter_restricted) setRestrictedChannel(channelMenu.channel); else void useAppStore.getState().joinChannel(channelMenu.channel.id); setChannelMenu(null); }}
        onToggleListen={() => { void useAppStore.getState().toggleListen(channelMenu.channel.id); setChannelMenu(null); }}
        onToggleNotifications={() => { void useAppStore.getState().toggleMutePushChannel(channelMenu.channel.id); setChannelMenu(null); }}
        onCreateSubchannel={() => { setChannelEditor({ channel: null, parentId: channelMenu.channel.id }); setChannelMenu(null); }}
        onEdit={() => { setChannelEditor({ channel: channelMenu.channel, parentId: channelMenu.channel.parent_id ?? 0 }); setChannelMenu(null); }}
        onEditPermissions={() => { void useAppStore.getState().selectChannel(channelMenu.channel.id); setSurface("admin"); setChannelMenu(null); }}
        onMove={(direction) => void reorderChannel(channelMenu.channel, direction)}
        onMoveAllUsers={() => { setMoveUsersSource(channelMenu.channel); setChannelMenu(null); }}
        onPurgeHistory={() => { setPurgeChannel(channelMenu.channel); setChannelMenu(null); }}
      />}
      {moveUsersSource && <MoveUsersDialog source={moveUsersSource} channels={channels} onClose={() => setMoveUsersSource(null)} />}
