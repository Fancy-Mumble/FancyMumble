...
export function filterVisibleChannels(input: {
  ...
  for (const channel of candidates) {
    ...
    const matchesVisibility = !input.hideEmpty || channel.user_count > 0
      || channel.id === input.currentChannel || channel.id === input.selectedChannel;
...
function asOfflineEntry(user: RegisteredUser): UserEntry {
  return {
    session: -(user.user_id + 1), name: user.name, channel_id: user.last_channel ?? 0,
    user_id: user.user_id, texture_size: user.texture_size ?? null, comment: user.comment,
...
export function listChannelMembers(input: {
  ...
  const offline = input.scope === "server"
...
export function filterChannelMessages<T extends RelatableMessage & { channel_id?: number | null; dm_session?: number | null }>(input: {
  messages: readonly T[];
  ...
  return [...input.messages, ...input.pollMessages].filter((message) =>
