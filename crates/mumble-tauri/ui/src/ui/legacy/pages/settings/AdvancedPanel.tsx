...
registerSettings("advanced")
  ...
  .add("advanced.webUiDesign", ["new ui", "interface", "design", "legacy"])
...
export function AdvancedPanel({
  ...
  useEffect(() => {
    void getSelectedUiDesign()
      .then((design) => setNewUi(design === "new"))
      .catch(() => undefined);
  }, []);
  ...
  const handleUiDesignToggle = async () => {
    ...
    try {
      await setSelectedUiDesign(next ? "new" : "legacy");
    } catch (error) {
      setNewUi(!next);
      console.error("failed to change WebUI design:", error);
    } finally {
  ...
  return (
    <>
      ...
      <section className={styles.section}>
        <div className={styles.toggleRow}>
          <div className={styles.toggleInfo}>
            <h3 className={styles.sectionTitle}>{t("advanced.webUiDesign")}</h3>
            <p className={styles.fieldHint}>
              {uiDesignOverride
                ? t("advanced.webUiDesignOverrideHint", { design: uiDesignOverride })
                : t("advanced.webUiDesignHint")}
            </p>
          </div>
          <Toggle
            checked={newUi}
            ...
            testId="advanced-new-ui-toggle"
            ariaLabel={t("advanced.webUiDesign")}
          />
        </div>
      </section>
