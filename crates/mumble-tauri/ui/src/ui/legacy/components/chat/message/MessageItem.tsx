...
import type { ActionRow } from "@core/plugins/tier1/types";
...
export default memo(function MessageItem({
  ...
  const renderBody = () => {
    ...
    const liveDocMatch = FANCY_LIVEDOC_MARKER_RE.exec(bodyWithoutQuotes);
    if (liveDocMatch) {
      const decoded = decodeLiveDocInviteMarker(liveDocMatch[1]);
      if (decoded) {
        return (
          <>
            {quoteBlocks}
            <LiveDocInviteCard
              channelId={msg.channel_id ?? 0}
              slug={decoded.slug}
              title={decoded.title}
