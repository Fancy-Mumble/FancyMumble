...
const user = (session: number, overrides: Partial<UserEntry> = {}): UserEntry => ({
  session, name: `user-${session}`, channel_id: 0, user_id: null, texture_size: null, comment: null,
...
describe("selectSelfDeafened", () => {
  it("reads self_deaf from our own user", () => {
    expect(selectSelfDeafened(state({ ownSession: 2, users: [user(1, { self_deaf: true }), user(2, { self_deaf: true })] }))).toBe(true);
  ...
  it("does not report someone else's deafen as our own", () => {
    expect(selectSelfDeafened(state({ ownSession: 2, users: [user(1, { self_deaf: true }), user(2)] }))).toBe(false);
  });

  it("is false before the server assigns a session", () => {
    expect(selectSelfDeafened(state({ ownSession: null, users: [user(1, { self_deaf: true })] }))).toBe(false);
  ...
  it("is independent of the voice state, which cannot express deafen", () => {
    ...
    expect(selectSelfDeafened(state({ ownSession: 1, voiceState: "inactive", users: [user(1)] }))).toBe(false);
    expect(selectSelfDeafened(state({ ownSession: 1, voiceState: "active", users: [user(1, { self_deaf: true })] }))).toBe(true);
  });
});
