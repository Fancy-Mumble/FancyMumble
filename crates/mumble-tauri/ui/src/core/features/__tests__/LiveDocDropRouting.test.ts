...
import { resolveDropTarget, useLiveDocDropStore } from "../chat/livedoc/liveDocDropStore";
...
describe("resolveDropTarget", () => {
  ...
  it("routes everything to the live doc when maximised (chat hidden)", () => {
    expect(resolveDropTarget({ mode: "max", point: { x: 10, y: 9999 }, liveDocRect: liveRect })).toBe(
      "livedoc",
    );
  ...
  describe("half mode uses geometry", () => {
    it("targets the live doc when the pointer is inside its rect", () => {
      expect(resolveDropTarget({ mode: "half", point: { x: 500, y: 150 }, liveDocRect: liveRect })).toBe(
        "livedoc",
      );
    });

    it("targets the chat when the pointer is below the live doc rect", () => {
      expect(resolveDropTarget({ mode: "half", point: { x: 500, y: 400 }, liveDocRect: liveRect })).toBe(
        "chat",
      );
    });
    ...
    it("includes the rect edges as inside the live doc", () => {
      expect(resolveDropTarget({ mode: "half", point: { x: 0, y: 0 }, liveDocRect: liveRect })).toBe(
        "livedoc",
      );
      expect(resolveDropTarget({ mode: "half", point: { x: 1000, y: 300 }, liveDocRect: liveRect })).toBe(
        "livedoc",
      );
    });
  });
