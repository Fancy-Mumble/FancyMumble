...
mod probe {
    ...
    unsafe fn run(want: Option<&str>) -> windows::core::Result<()> {
        // SAFETY: `main` has initialized COM on this thread; every COM result
        ...
        unsafe {
            ...
            println!("\nInterpretation:");
            println!(
                "  SHARED failing 'in use' + EXCLUSIVE OK -> add exclusive-mode capture (like Mumble)."
            );
    ...
    unsafe fn select_device(
        ...
        // SAFETY: the caller has initialized COM on this thread; every COM result
        // is checked before it is used.
        unsafe {
    ...
    unsafe fn probe_shared(device: &IMMDevice) -> windows::core::Result<u32> {
        // SAFETY: the caller has initialized COM on this thread; the mix format
        // pointer comes from `GetMixFormat` and is freed with `CoTaskMemFree`.
        unsafe {
            ...
            match client.Initialize(
                ...
                Err(e) => println!(
                    "WASAPI SHARED + mix format: FAILED hr=0x{:08X} ({})",
                    e.code().0,
                    e.message()
                ),
            }
    ...
    unsafe fn probe_exclusive(device: &IMMDevice, rate: u32) -> windows::core::Result<()> {
        ...
        // every `Initialize` call that borrows it.
        unsafe {
            for &(ch, bits) in &[(2u16, 16u16), (1, 16), (2, 24), (1, 24), (2, 32), (1, 32)] {
                ...
                if client
                    .IsFormatSupported(AUDCLNT_SHAREMODE_EXCLUSIVE, &wfe, None)
                    .is_err()
                {
                    continue;
                }
